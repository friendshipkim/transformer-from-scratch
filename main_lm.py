...
def get_batch(source: Tensor, i: int) -> Tuple[Tensor, Tensor]:
    ...
    """
    seq_len = min(cfg.src_seq_len, len(source) - 1 - i)
    # if shorter than max_seq_len, padding
    if seq_len < cfg.src_seq_len:
...
def train(model: nn.Module) -> None:
    ...
    num_batches = len(train_data) // cfg.src_seq_len
    print("num_batches:", num_batches)
    for batch, i in enumerate(range(0, train_data.size(0) - 1, cfg.src_seq_len)):
        # print("batch", batch, "i", i)
        data, targets = get_batch(train_data, i)
        # truncate last batch
        if data.size(1) < cfg.src_seq_len:
            print("pass last batch")
            continue
        batch_size = data.size(0)
        output = model(data, data)[-1]
        breakpoint()
        print("output shape", output.view(-1, vocab_size).shape)  # TODO: why one input
...
if __name__ == "__main__":
    ...
    forward_result = model(test_src_input, test_tgt_input)[-1]
    # test if output shape is correct
    assert forward_result.shape == (cfg.batch_size, cfg.src_seq_len, vocab_size)
